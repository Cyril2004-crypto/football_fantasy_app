  ...
  Future<void> initialize() async {
    try {
      ...
      final vapidKey = kIsWeb && hasWebVapidKey
          ...
          : null;
      debugPrint(
        '🔑 Using VAPID key: ${vapidKey != null ? "YES (${vapidKey.length} chars)" : "NO"}',
      );

      if (kIsWeb && !hasWebVapidKey) {
        debugPrint(
          '⚠️ Web VAPID key is empty. Add FIREBASE_WEB_VAPID_KEY in dart_defines.local.json.',
        );
        return;
      }
      ...
      try {
        ...
        debugPrint(
          '📋 ⬆️ Copy this token to Firebase Console to send test notifications',
        );
      } on MissingPluginException catch (e) {
        debugPrint('❌ FCM plugin not registered on this run: $e');
        debugPrint(
          'ℹ️ Close all flutter run sessions, close Chrome, then run flutter clean; flutter pub get; flutter run -d chrome --dart-define-from-file=dart_defines.local.json',
        );
      }
    } catch (e) {
