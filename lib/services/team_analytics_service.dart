...
class TeamAnalyticsService {
  ...
  Future<TeamAnalytics> analyzeTeam({
    ...
    try {
      ...
      final transferRecommendations =
          await _fetchTransferRecommendations(players);

      // Calculate team form score (0-100)
      final teamFormScore =
          _calculateTeamFormScore(formTrends, injuryRisks);
  ...
  Future<List<FormTrend>> _fetchFormTrends(
    ...
    try {
      ...
      final response = await _supabase
          ...
          .or('player_id.in.(SELECT id FROM public.fd_players WHERE team_id=$teamId)')
      ...
      final trends = groupedByGameweek.entries
          .map((e) {
            final gameweek = e.key;
            final points = e.value;
            final average =
                points.isEmpty ? 0 : points.reduce((a, b) => a + b) / points.length;
      ...
      print('Error fetching form trends: $e');
      return [];
    }
  }
  ...
  Future<List<InjuryRisk>> _fetchInjuryRisks(List<dynamic> players) async {
    try {
      final playerIds =
          players.map((p) => (p as Map).tryGet('id')).whereType<int>().toList();
      ...
      for (final injury in injuriesResponse as List) {
        ...
        if (injury['expected_return_date'] != null) {
          ...
          injuriesMap[playerId]!
              .add(DateTime.parse(injury['expected_return_date'] as String));
        }
      }
      ...
      for (final suspension in suspensionsResponse as List) {
        ...
        if (suspension['expected_return_date'] != null) {
          ...
          suspensionsMap[playerId]!
              .add(DateTime.parse(suspension['expected_return_date'] as String));
        }
      }
      ...
      final risks = players
          .map((p) {
            ...
            if (injuriesMap[playerId] != null && injuriesMap[playerId]!.isNotEmpty) {
              expectedReturnDate = injuriesMap[playerId]!.reduce(
                (a, b) => a.isAfter(b) ? a : b,
              );
            }
            if (suspensionsMap[playerId] != null && suspensionsMap[playerId]!.isNotEmpty) {
              final suspensionReturn = suspensionsMap[playerId]!.reduce(
                (a, b) => a.isAfter(b) ? a : b,
              );
              if (expectedReturnDate == null || suspensionReturn.isAfter(expectedReturnDate)) {
      ...
      print('Error fetching injury risks: $e');
      return [];
    }
  }
    ...
    try {
      ...
      final recommendations = players
          .map((p) {
            ...
            final priority =
                (estimatedValue / (estimatedPrice + 0.001)).clamp(1, 5).toInt();
      ...
      print('Error fetching transfer recommendations: $e');
      return [];
    }
  }

  double _calculateTeamFormScore(
    ...
    // Reduce score based on injury risk
    final highRiskPlayers = injuryRisks.where((r) => r.riskLevel == 'high').length;
