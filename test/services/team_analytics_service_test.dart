    ...
    test('buildInjuryRisksFromInputs scores player availability risk', () {
      final risks = buildInjuryRisksFromInputs(
        ...
        injuriesResponse: [
          {
            'player_id': 1,
            'expected_return_date': '2026-04-20T00:00:00Z',
          },
        ],
        suspensionsResponse: [
          {
            'player_id': 2,
            'expected_return_date': '2026-04-18T00:00:00Z',
          },
        ],
      );
      ...
      expect(risks.first.expectedReturnDate, DateTime.parse('2026-04-20T00:00:00Z'));
      expect(risks.last.playerId, 2);
      expect(risks.last.riskScore, 20);
    });

    test('buildTransferRecommendationsFromInputs returns buy and sell recommendations', () {
      final recommendations = buildTransferRecommendationsFromInputs(
        players: [
          {'id': 1, 'name': 'Star Midfielder', 'position': 'midfielder'},
          {'id': 2, 'name': 'Underperforming Defender', 'position': 'defender'},
        ],
        statsResponse: [
      ...
      expect(recommendations.first.priority, greaterThanOrEqualTo(recommendations.last.priority));
    });

    test('calculateTeamFormScoreFromInputs applies injury penalty', () {
      final score = calculateTeamFormScoreFromInputs(
        [
          FormTrend(gameweek: 1, points: 10, windowAverage: 10, trend: 'stable'),
